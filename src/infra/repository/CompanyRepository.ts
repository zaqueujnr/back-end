...
export class CompanyRepositoryDatabase implements CompanyRepository {
    ...
    async existsByEmail(email: string): Promise<boolean> {
        const existsEmail = await this.connection.query("SELECT * FROM company WHERE email = $1",
            [email]);
    ...
    async getCompanies(params: any = {}): Promise<Output> {
        ...
        query += ` ORDER BY company_id DESC LIMIT $${values.length + 1} OFFSET $${values.length + 2}`;
        values.push(limit, offset)
        
        const result = await this.connection.query(query, values)

        const companies = result.map((company: any) =>
            new Company(company.company_id, company.name, company.cnpj, company.email, company.endereco)
        )

        const totalCompanies = await this.connection.query(countQuery, values.slice(0, -2))
