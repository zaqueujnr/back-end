...
export class WorkRepositoryDatabase implements WorkRepository {
    ...
    async saveWork(work: Work): Promise<void> {
        return await this.connection.query("insert into work (work_id, description, date_init, date_end, type_contract, time) values ($1, $2, $3, $4, $5, $6)",
            [work.workId, work.description, work.dateInit, work.dateEnd, work.typeContract, work.time]
        )
    }
    ...
    async getWorks(params: any = {}): Promise<Output> {
        ...
        const works = result.map((work: any) => {
            const dateInit = new Date(work.date_init).toISOString()
            const dateEnd = new Date(work.date_init).toISOString()
            return new Work(work.work_id, work.description, dateInit, dateEnd, work.type_contract, work.time, work.company_id)
        })
