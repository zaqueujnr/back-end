...
export class ProfessionalRepositoryDatabase implements ProfessionalRepository {
  ...
  async saveProfessional(professional: Professional): Promise<string> {
    return await this.connection.query(
      "insert into professional (professional_id, name, email, position, salary) values ($1, $2, $3, $4, $5)",
      [
  ...
  async getProfessionals(params: any = {}): Promise<Output> {
    ...
    query += ` ORDER BY professional_id DESC LIMIT $${
      values.length + 1
    } OFFSET $${values.length + 2}`;
    values.push(limit, offset);

    const result = await this.connection.query(query, values);
    const professionals = result.map(
      (professional: any) =>
        new Professional(
    ...
    const totaProfessionals = await this.connection.query(
      countQuery,
      values.slice(0, -2)
    );
